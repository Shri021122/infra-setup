...
phase2_terraform_proxmox() {
  ...
  SSH_KEY=$(jq -r '.vm_ssh_private_key_path.value // "~/.ssh/id_rsa"' "${SECRETS_DIR}/tf-outputs.json" 2>/dev/null || echo "${ROOT_DIR}/.secrets/cluster_id_ed25519")
  SSH_USER=$(grep 'vm_user' terraform.tfvars | cut -d= -f2 | tr -d ' "' || echo "ubuntu")

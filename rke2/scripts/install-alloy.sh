...
# ─── Resolve the active cluster from inventory.ini ────────────────────────────
# inventory.ini is regenerated by `terraform apply` on every deploy and carries
# cluster_name in its [all:vars] section. The per-cluster observability tfvars
# we need then lives at clusters/<cluster_name>/observability.tfvars.
INVENTORY="${CONFIGS_DIR}/inventory.ini"
CLUSTER_NAME=""
if [[ -f "$INVENTORY" ]]; then
  CLUSTER_NAME=$(grep '^cluster_name=' "$INVENTORY" 2>/dev/null | head -1 | cut -d= -f2 || true)
fi
[[ -n "$CLUSTER_NAME" ]] || { echo "[alloy] ERROR: cluster_name not found in $INVENTORY"; exit 1; }

OBS_TFVARS="${ROOT_DIR}/clusters/${CLUSTER_NAME}/observability.tfvars"
[[ -f "$OBS_TFVARS" ]] || { echo "[alloy] ERROR: observability tfvars not found at $OBS_TFVARS"; exit 1; }

# ─── Load centralized endpoints from the per-cluster observability tfvars ────
read_tfvar() {
  local key="$1"
  # Tolerant of missing keys — return empty rather than triggering pipefail.
  { grep "^${key}" "$OBS_TFVARS" 2>/dev/null || true; } | head -1 | \
